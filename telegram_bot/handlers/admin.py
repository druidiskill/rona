...
from database import admin_repo, service_repo, booking_repo, client_repo
...
async def admin_bookings(callback: CallbackQuery, is_admin: bool):
    ...
    bookings = await booking_repo.get_by_date_range(today, week_later)
    ...
    for booking in bookings[:10]:  # Показываем первые 10
        bookings_text += f"#{booking.id} - {booking.start_time.strftime('%d.%m %H:%M')}\n"
        bookings_text += f"Статус: {booking.status.value}\n"
        bookings_text += f"Цена: {booking.all_price}₽\n\n"
...
async def bookings_today(callback: CallbackQuery, is_admin: bool):
    ...
    bookings = await booking_repo.get_by_date_range(today, tomorrow)
    ...
    for booking in bookings:
        bookings_text += f"🕐 {booking.start_time.strftime('%H:%M')}\n"
        bookings_text += f"👥 {booking.num_clients} чел.\n"
        bookings_text += f"💰 {booking.all_price}₽\n"
        bookings_text += f"📊 {booking.status.value}\n\n"
...
async def bookings_tomorrow(callback: CallbackQuery, is_admin: bool):
    ...
    bookings = await booking_repo.get_by_date_range(tomorrow, day_after)
